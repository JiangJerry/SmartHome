...
extern FlagStatus TaskWeightRdy,TaskRFRdy,TaskLEDRdy,TaskKeyRdy,TaskADCRdy,TaskBuzzerRdy;
void LedCallBack(void);
extern u8 LedFlash;
u8 SFlag = 0;
u16 CntForLockTime = 0;
extern __FUNC_STRUCT Func;
FlagStatus WissGetFlag = RESET;
u8 RFDelayLedOff = 0;
...
{
    static u8 SameCnt = 0,Temp = 0;
    if (EXTI_GetEdgeStatus(KEY1_BUTTON_EXTI_CHANNEL, EXTI_EDGE_POSITIVE))
    {
        ...
        switch(RFCtrl.Sta)
        {
            ...
            case STA_DATA:      //收数据
                ...
                if(RFCtrl.Cnt >= 24)
                {
					if((RFCtrl.Data[0] == SaveID[0])&&(RFCtrl.Data[1] == SaveID[1]))
					{
						RFDelayLedOff = 5;
					}
...
//                        if(RFCtrl.ForbidTime == 0)
    ...
    else if(EXTI_GetEdgeStatus(KEY1_BUTTON_EXTI_CHANNEL, EXTI_EDGE_NEGATIVE))
    {
        EXTI_ClearEdgeFlag(KEY1_BUTTON_EXTI_CHANNEL);
        TSCnt = 0;
    }
...
void ADC_IRQHandler(void)
{
  if (ADC_GetIntStatus(HT_ADC, ADC_INT_CYCLE_EOC) == SET)
  {
    ...
    ADSw.Valu  = ((HT_ADC->DR[0]&0x0FFF));
    StopDelay.Valu       = ((HT_ADC->DR[1]&0x0FFF));
  }    
}
/*********************************************************************************************************//**
